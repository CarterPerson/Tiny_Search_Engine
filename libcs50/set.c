...
This code started out as copied and pasted from set.h, so many of the comments are not my own
 ...
 */
set_t *set_new(void){
    set_t *returner = (set_t *)calloc(1, sizeof(set_t));
    if(returner == NULL){
        return NULL;
    }
    returner->length = 0;
...
bool set_insert(set_t *set, const char *key, void *item){
    ...
    setnode_t *adder = setnode_new(item, key);
    adder->next = set->head;
    set->head = adder;
    set->length++;
    return true;
